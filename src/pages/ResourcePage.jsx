...
export default function ResourcePage({ title, description, api, idKey, columns, createFields, updateFields, emptyMessage, module, detailsPath }) {
  ...
  const session = getSession(); const role = session.role
  ...
  const submit = async (event) => {
    ...
    const nextErrors = Object.fromEntries(fields.filter((field) => field.required && (form.values[field.name] === undefined || form.values[field.name] === '')).map((field) => [field.name, `${field.label} is required.`]))
  ...
  const begin = (mode, item = {}) => { setErrors({}); setForm({ mode, values: { ...item } }) }
