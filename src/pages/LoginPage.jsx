...
export default function LoginPage() {
  ...
  const submit = async (event) => { event.preventDefault(); try { const result = await request.run(() => login(form), 'Signed in'); const token = result.data?.accessToken; if (token) localStorage.setItem('wsr_access_token', token); if (result.data?.tokenType) localStorage.setItem('wsr_token_type', result.data.tokenType); if (result.data?.userId) localStorage.setItem('wsr_user_id', String(result.data.userId)); if (result.data?.role) localStorage.setItem('wsr_role', result.data.role); navigate('/dashboard') } catch { /* displayed below */ } }
